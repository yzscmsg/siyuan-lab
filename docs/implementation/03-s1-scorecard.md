...
| S1-D3 | Medium | `core.document` has no `status`/`version`/`supersedes` column; week-4 step 3 lists status as required. Withdrawal is only expressible in `metadata` jsonb, which no constraint can enforce. | See `retraction_report.json`; enforce in the week-4 API layer or add a constraint migration. |
| S1-D4 | Medium | `core.document` accepts any `media_type` (no CHECK) and does not validate `storage_uri` (path traversal possible). | Enforce in the week-4 Document API layer (not-yet-implemented). |
